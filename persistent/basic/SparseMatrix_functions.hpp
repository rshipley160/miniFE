...
// Note - only used if MINIFE_FUSED is defined - no need to change
template<typename MatrixType,
         typename VectorType>
...
// Replace with persistent initialization; make sure this is called instead of 
template<typename MatrixType,
         typename VectorType>
struct matvec_std {
void operator()(MatrixType& A,
            ...
            VectorType& y)
{
  std::cout << "Using matvec_std basic" << std::endl;
                    ...
                    VectorType& y)
{
  ...
  std::cout << "Using matvec_overlap basic" << std::endl;
